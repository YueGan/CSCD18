...
//  loadTexture(o,"./star.ppm",1,&texture_list);
...
//  loadTexture(o,"./star.ppm",1,&texture_list);
 ...
 o=newSphere(0.0,1.0,0.0,.75,.75,.75,.02,1.4);	// Bottom
 Scale(o,500,500,500);
 Translate(o,0,-510,5);
 invert(&o->T[0][0],&o->Tinv[0][0]);
 insertObject(o,&object_list);

 o=newSphere(0.0,1.0,0.0,.75,.75,.75,.05,1.4);		// Top
 Scale(o,500,500,500);
 Translate(o,0,510,5);
 invert(&o->T[0][0],&o->Tinv[0][0]);
 insertObject(o,&object_list);
 ...
 // Two spheres scene
//  o=newSphere(0.0,0.3,.7,.99,.99,.99,.01,1.54);		// Refract
...
//  }
  ...
  for (int i=0; i<17;i++)
 {
  o=newSphere(i/17,1.0 - i/17,0.0,.99,.99,.99,.01,2.00+(.05*i));
  Scale(o,1.35,1.35,1.35);
  Translate(o,7.60*cos(2*PI*i/7),-6.35 + i*0.8,3+4.60*sin(2*PI*i/7));
  invert(&o->T[0][0],&o->Tinv[0][0]);
  insertObject(o,&object_list);
 }
